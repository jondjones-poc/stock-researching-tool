...
try {
  ...
  const sql = readFileSync(join(__dirname, 'migrations', '047_portfolio_style_tags.sql'), 'utf8');
  await client.query(sql);
  const categories = await client.query(
    `SELECT slug, label, sort_order FROM portfolio_style_categories ORDER BY sort_order, slug`
  );
  console.log('Applied 047_portfolio_style_tags.sql');
