...
try {
  ...
  const path = join(__dirname, 'migrations', '023_portfolio_stocks.sql');
  if (existsSync(path)) {
    await runFile('023_portfolio_stocks.sql');
  }

  const result = await client.query(
    `SELECT ps.id, sv.stock AS symbol
  ...
  if (result.rows.length === 0) {
    console.log('  (none yet — add from /stocks → My Portfolio)');
  } else {
