...
describe('/api/earnings-growth - Final Real Data Tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should calculate correct growth rate for CAKE with real data (including negative EPS)', async () => {
    ...
    // Our algorithm: moving from negative to positive EPS should show positive growth
    const expectedGrowth = Math.abs(Math.pow(Math.abs(3.28) / Math.abs(-6.32), 1/4) - 1);
    expect(data.historicalGrowthRate).toBeCloseTo(expectedGrowth, 3);
  ...
  it('should demonstrate growth calculation formulas with correct values', () => {
    ...
    // CAKE: Mixed positive/negative EPS values (using absolute values and taking absolute result)
    const cakeLatest = 3.28;
    const cakeOldest = -6.32;
    const cakeYears = 4;
    const cakeGrowth = Math.abs(Math.pow(Math.abs(cakeLatest) / Math.abs(cakeOldest), 1/cakeYears) - 1);
  ...
  it('should verify exact values from saved JSON files', () => {
    ...
    const cakeCalculated = Math.abs(Math.pow(Math.abs(3.28) / Math.abs(-6.32), 1/4) - 1);
