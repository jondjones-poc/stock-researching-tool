...
import { clearResearchSymbolLinksCache } from '@/app/utils/researchSymbolLinks';

async function hasResearchSymbolColumn(): Promise<boolean> {
  try {
...
export async function GET() {
  try {
    ...
    const hasResearchSymbol = await hasResearchSymbolColumn();
    const cols = [
      'instrument_id',
      'symbol_full',
      ...(hasResearchSymbol ? ['research_symbol'] : []),
      'display_name',
      'exchange',
      'type',
      'updated_at',
      'dividend_per_share',
      ...(hasIsDividend ? ['is_dividend'] : []),
    ].join(', ');
    ...
    const rows = result.rows.map((r: Record<string, unknown>) => ({
      ...
      ...(hasResearchSymbol ? { researchSymbol: r.research_symbol ?? '' } : {}),
    ...
    return NextResponse.json({ rows, hasResearchSymbol });
  } catch (e: unknown) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const allowed: Record<string, string> = {
      ...
      researchSymbol: 'research_symbol',
    ...
    for (const [key, col] of Object.entries(allowed)) {
      ...
      if (key === 'researchSymbol' && !hasResearchSymbol) continue;
      ...
      } else if (key === 'researchSymbol') {
        setParts.push(`${col} = $${idx}`);
        const s = val === null || val === undefined ? '' : String(val).trim().toUpperCase();
